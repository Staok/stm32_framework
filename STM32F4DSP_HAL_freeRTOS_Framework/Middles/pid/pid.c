...
pid_realize pid[pidDeviceNum] = 
{
	{
		{ /*Moto1参数初始化*/
			0.0f, 		/* SetSpeed，定义设定值：MaxSpeed ~ MinSpeed*/
			0.0f,		/* ActualSpeed,定义实际值：MaxSpeed ~ MinSpeed*/
			
			/*用户改此处*/
			300.0f,		/* MaxSpeed,MinSpeed,定义最大、最小速度，用于归一化：执行器接受的最小值~执行器接受的最大值*/
			-300.0f,		/* 最大、小速度一般都关于0对称，MaxSpeed必须大于0，MinSpeed必须小于0*/
			/*用户改此处*/
			0.7f,		/* Kp,Ki,Kd,定义比例、积分、微分系数：0~1.0*/
			0.2f,
			0.05f,
			/*用户改此处*/
			0.5f,		/* isoKi_Max,积分分离的上限：0~1.0*/
			0.2f,		/* isoKi_Min,积分分离的下限：0~1.0*/
			
			0.0f,		/* output,定义计算结果，执行器的输入：-1.0~1.0*/
			0.0f,		/* err,定义偏差值*/
			0.0f,		/* err_last,定义上一个偏差值*/
			0.0f		/* integral,定义积分值*/
		},	Moto1PID_Realize
	},
	{
...
float Moto1PID_Realize(pid_struct* pid)
{
	...
	/*抗积分饱和过程*/
	if(pid->ActualSpeed > (pid->MaxSpeed * 0.7f)) /*这个时候积分项只进行负误差的累加*/
	{
		/*变积分过程*/
		if(pid->err >= 0) absErr = pid->err; else absErr = -pid->err; /*取绝对值*/
		if(absErr > pid->isoKi_Max) 
		{
			index = 0.0f; /*大误差时，不使用积分项*/
		}else if(absErr < pid->isoKi_Min){ 
			index = 1.0f; /*误差小于积分分离下限时，使用完全积分项*/
	...
	}else if(pid->ActualSpeed < (pid->MinSpeed * 0.7f)) /*这个时候积分项只进行正误差的累加*/
	{
	...
	/*防超范围*/
	if(pid->output > 1)  {pid->output = 1.0f;}
	if(pid->output < -1) {pid->output = -1.0f;}
