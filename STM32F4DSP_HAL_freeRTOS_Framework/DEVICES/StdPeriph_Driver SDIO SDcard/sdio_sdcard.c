...
/*
直接看.h文件里对外调用的API：
	extern SD_CardInfo SDCardInfo;				//SD卡信息（初始化时自动获取）
	SD_Error SD_Init(void);						//用户用，一个函数初始化SDIO驱动的SD卡，fatfs调用
	u8 SD_ReadDisk(u8*buf,u32 sector,u8 cnt); 	//用户用，读SD卡,fatfs/usb调用
	u8 SD_WriteDisk(u8*buf,u32 sector,u8 cnt);	//用户用，写SD卡,fatfs/usb调用
*/
