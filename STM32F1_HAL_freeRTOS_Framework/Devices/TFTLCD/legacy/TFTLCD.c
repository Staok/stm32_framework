...
{  
	...
	LCD_CS_SETLCD_RS_SETLCD_WR_SETLCD_RD_SETLCD_RST_SET
...
void LCD_Clear(u32 color)
{
		...
		LCD_WriteRAM_Prepare()     		//开始写入GRAM	 	  
		for(index=0index<totalpointindex++)
		{
			LCD->LCD_RAM=color	
		}
	#else
		...
		LCD_RS_SET//写数据 
		LCD_CS_CLR
		for(index=0index<lcddev.width*lcddev.heightindex++)
		{
...
{     
		...
		for(i=syi<=eyi++)
		{
			...
			LCD_WriteRAM_Prepare()     			//开始写入GRAM	  
			for(j=0j<xlenj++)LCD->LCD_RAM=color	//显示颜色 	    
		}  
	#else
		u16 i,j			
...
{  
		...
		for(i=0i<heighti++)
		{
			...
			LCD_WriteRAM_Prepare()     //开始写入GRAM
			for(j=0j<widthj++)LCD->LCD_RAM=color[i*width+j]//写入数据 
		} 
	#else
...
void LCD_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2)
{
	...
	else {incx=-1delta_x=-delta_x} 
	if(delta_y>0)incy=1 
	else if(delta_y==0)incy=0//水平线 
	else{incy=-1delta_y=-delta_y} 
	if( delta_x>delta_y)distance=delta_x //选取基本增量坐标轴 
	else distance=delta_y 
	for(t=0t<=distance+1t++ )//画线输出 
	{  
