...
编程和应用规范：
	...
	No.2: 尽量减少数据传输过程中的拷贝，常用指针、结构体指针当传输数据，函数形参带const修饰符
	No.3: 一类或者一个模块内的变量尽量用结构体打包，多组数据就用结构体数组
		  函数返回值0表示成功，正数表示失败，此正数可以表示错误代码
		  IO尽量设计为低电平有效，高电平截止；按键IO尽量都使用外部中断
		  大块内存使用请用本模板内存管理相关API
		  
		  外设相关的所有API均在：
				#include "sys.h"
				里面，APP编写时按需要在.c文件内调用这个即可
		独立IO口相关的所有API均在：
...
/*
命名规范：（临时）
	0、函数命名规则为“属什么是什么做什么”，如：sys_irq_disable()，属于sys级别函数，是irq管理，做dsiable的功能
...
/*本模板默认和规范的事项：（临时，待整理）
	00、工程内现已经不含string、stdio和stdlib等不适合用于mcu平台的头文件，已经都用开源的线程安全软件或者自实现替换
		（对于FreeRTOS包含stdlib库、stdio库和string库，没办法）
	0、中断优先级分组选用分组4，即16级抢占优先级，0级响应优先级（即不用）
	1、IO的低电平为有效电平，高电平截止或者无效；按键IO尽量都使用外部中断
	2、使用C99模式编译！
	3、I2C使用软件实现
...
/*可用的API： 目前串口接收的问题就是一次涌入数据过多会崩，可能是存储数据时栈溢出，但是暂时找不到在哪里，小数据量接收没大问题
	...
	接收：（以下只针对串口1说明）（一次接收字符不要超过200个字节，如果超出则丢弃之后的数据）
			...
			最新：串口1的接收协议1目前不可用，等以后加上可以解析别的协议如modbus等等
*/
